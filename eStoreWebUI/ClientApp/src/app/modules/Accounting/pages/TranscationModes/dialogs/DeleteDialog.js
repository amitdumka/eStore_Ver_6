...
export function DeleteDialog({ id, show, onHide }) {
  ...
  const deleteTranscationMode = () => {
    // server request for deleting transcationMode by id
    dispatch(actions.deleteTranscationMode(id)).then(() => {
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">
          TranscationMode Delete
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {!isLoading && (
          <span>Are you sure to permanently delete this transcationMode?</span>
        )}
        {isLoading && <span>TranscationMode is deleting...</span>}
      </Modal.Body>
      <Modal.Footer>
        <div>
          <button
