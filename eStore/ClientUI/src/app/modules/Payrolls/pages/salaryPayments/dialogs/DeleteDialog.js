...
export function DeleteDialog({ id, show, onHide }) {
  ...
  const deleteSalaryPayment = () => {
    // server request for deleting salaryPayment by id
    dispatch(actions.deleteSalaryPayment(id)).then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Body>
        {!isLoading && (
          <span>Are you sure to permanently delete this salaryPayment?</span>
        )}
        {isLoading && <span>SalaryPayment is deleting...</span>}
      </Modal.Body>
      <Modal.Footer>
        <div>
          <button
