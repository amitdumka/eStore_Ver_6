...
export const customersSlice = createSlice({
  ...
  reducers: {
    ...
    customerDeleted: (state, action) => {
      state.error = null;
      state.actionsLoading = false;
      state.entities = state.entities.filter(el => el.customerId !== action.payload.customerId);
    },
    // deleteCustomers
    customersDeleted: (state, action) => {
      ...
      state.entities = state.entities.filter(
        el => !action.payload.ids.includes(el.customerId)
      );
    },
    ...
    customersStatusUpdated: (state, action) => {
      ...
      state.entities = state.entities.map(entity => {
        if (ids.findIndex(id => id === entity.customerId) > -1) {
